...
export default class NonceFinder {
    constructor(params = {}) {
        ...
        if (this._workersCount == 'auto') {
            this._workersCount = this.guessWorkersCount();
        }
        
        console.log(''+this._name+' | Nonce Finder, threads: '+this._workersCount);
    }

    guessWorkersCount() {
        try {
            const cpus = os.cpus();
            console.log(''+this._name+' | system CPU count: '+cpus.length);
            return cpus.length;
        } catch (e) {
            return 8;
        }
